...
<head>
    ...
    <title>Edit Data Penguji Sidang</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-6 flex flex-col items-center justify-center">
            <div class="bg-white p-6 rounded-lg shadow-lg w-1/2">
                <h2 class="text-center text-xl font-bold">Edit Data Penguji Sidang</h2>
                ...
                <form action="/penguji_admin/{{ $data['id_penguji'] }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        ...
                        <select id="id_jadwal" name="id_jadwal" class="select2 w-full">
                            ...
                            @foreach ($jadwal as $jd)
                                <option value="{{ $jd['id_jadwal'] }}"
                                    {{ old('id_jadwal', $data['id_jadwal']) == $jd['id_jadwal'] ? 'selected' : '' }}>
                                    {{ $jd['id_jadwal'] }} - {{ $jd['npm'] ?? '' }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            <select id="nidn" name="nidn" class="select2 w-full">
                                ...
                                @foreach ($dosen as $dsn)
                                    <option value="{{ $dsn['nidn'] }}"
                                        {{ old('nidn', $data['nidn']) == $dsn['nidn'] ? 'selected' : '' }}>
