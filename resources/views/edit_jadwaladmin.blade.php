...
<head>
    ...
    <title>Edit Data Jadwal Sidang</title>
    ...
    <div class="flex h-screen">
        ...
        <div class="flex-1 p-6 flex flex-col items-center justify-center">
            <div class="bg-white p-6 rounded-lg shadow-lg w-1/2">
                <h2 class="text-center text-xl font-bold">Edit Data Jadwal Sidang</h2>
                ...
                <form action="/jadwal_admin/{{ $data['id_jadwal'] }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label class="block mb-1">NPM :</label>
                        <select id="npm" name="npm" class="select2 w-full">
                            <option value="">-- Pilih NPM Mahasiswa --</option>
                            @foreach ($mahasiswa as $mhs)
                                <option value="{{ $mhs['npm'] }}"
                                    {{ old('npm', $data['npm']) == $mhs['npm'] ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="kode_ruangan" name="kode_ruangan" class="select2 w-full">
                            <option value="">-- Pilih Ruangan --</option>
                            @foreach ($ruangan as $r)
                                <option value="{{ $r['kode_ruangan'] }}"
                                    {{ old('kode_ruangan', $data['kode_ruangan']) == $r['kode_ruangan'] ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" id="waktu_sidang" name="waktu_sidang"
                            class="select2 w-full border p-2 rounded"
                            value="{{ isset($data['waktu_sidang']) ? $data['waktu_sidang'] : old('waktu_sidang') }}">
                        </select>
                    </div>
                    <div class="flex justify-end gap-4 mt-4">
                        <a href="/jadwal_admin" class="bg-gray-400 hover:bg-gray-500 text-white py-2 px-4 rounded">←
                            Kembali</a>
                        <button type="submit"
