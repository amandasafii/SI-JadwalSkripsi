            ...
            <div class="max-w-7xl w-full mx-auto bg-white shadow-lg rounded-lg p-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
                    <div>
                        ...
                        <a href="/dosen_admin/create"
                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded inline-block">+
                            Tambah Data</a>
                    </div>

                    <!-- Form Pencarian -->
                    <form action="{{ url('/dosen_admin') }}" method="GET" class="w-full md:w-auto">
                        <input type="text" name="search" placeholder="Cari NIDN / Nama / Prodi"
                    ...
                    <table class="w-full border border-gray-400">
                        <thead>
                            <tr class="bg-gray-200">
                                ...
                                <th class="border border-gray-400 px-4 py-2">Program Studi</th>
                        ...
                        <tbody>
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td class="border border-gray-400 px-4 py-2">{{ $item['nidn'] }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $item['nama_dosen'] }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $item['program_studi'] }}</td>
                                    <td class="border border-gray-400 px-4 py-2">{{ $item['email'] }}</td>
                                    <td class="border border-gray-400 px-4 py-2 text-center w-24">
                                        <a href="{{ url('dosen_admin/' . $item['nidn']) }}/edit"
                                            class="text-blue-500 hover:text-blue-700 px-2">✏️</a>|
                                        <form id="delete-form-{{ $item['nidn'] }}"
                                            action="{{ url('dosen_admin/' . $item['nidn']) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmDelete('{{ $item['nidn'] }}')"
        ...
        function confirmDelete(nidn) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('delete-form-' + nidn).submit();
                }
            });
        }
